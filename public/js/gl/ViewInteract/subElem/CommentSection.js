...
CommentSection.prototype.updateMeta = function (obj) {
    ...
    } else if (this.pickedCommentType === 'proposalObject') {
        var list = thatGUI.openProposalList;
    } else if (this.pickedCommentType === 'geoPosObject') {
    ...
    if (list) {
        ...
        for (var j = 0; i < tds.length; i++) {
            if (tds[j].id === 'value') {
                ...
                switch (j) {
                    case 1:
                        tds[j].innerHTML = '<p>' + rawObj.name + '</p>';
                        break;
                    case 3:
                        if (rawObj.URL) {
                            tds[j].innerHTML = '<p>' + rawObj.URL.split('/')[rawObj.URL.split('/').length - 1] + '</p>';
                        } else {
                            tds[j].innerHTML = '<p>-</p>';
                        }
                        break;
